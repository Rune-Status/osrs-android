		...
		if (var0.type == 2) {
			...
			for (var4 = 0; var4 < var0.height; ++var4) {
				for (int var6 = 0; var6 < var0.width; ++var6) {
					int var7 = (var0.paddingX + 32) * var6;
					int var8 = (var0.paddingY + 32) * var4;
					if (var5 < 20) {
						var7 += var0.inventoryXOffsets[var5];
						var8 += var0.inventoryYOffsets[var5];
					}

					if (var1 >= var7 && var2 >= var8 && var1 < var7 + 32 && var2 < var8 + 32) {
						Client.dragItemSlotDestination = var5;
						FloorDecoration.field1604 = var0;
						if (var0.itemIds[var5] > 0) {
							ItemDefinition var9 = HealthBarDefinition.ItemDefinition_get(var0.itemIds[var5] - 1);
							if (Client.isItemSelected == 1 && WorldMapManager.method673(WorldMapIcon_0.getWidgetClickMask(var0))) {
								if (var0.id != FriendSystem.selectedItemWidget || var5 != class65.selectedItemSlot) {
									GraphicsObject.insertMenuItemNoShift("Use", Client.selectedItemName + " -> " + PacketWriter.colorStartTag(16748608) + var9.name, 31, var9.id, var5, var0.id);
								}
								...
								if ((NetSocket.selectedSpellFlags & 16) == 16) {
									GraphicsObject.insertMenuItemNoShift(Client.selectedSpellActionName, Client.selectedSpellName + " -> " + PacketWriter.colorStartTag(16748608) + var9.name, 32, var9.id, var5, var0.id);
								}
							} else {
								...
								if (WorldMapManager.method673(WorldMapIcon_0.getWidgetClickMask(var0))) {
									for (var12 = 4; var12 >= 3; --var12) {
										if (var11 != var12) {
											GrandExchangeEvents.addWidgetItemMenuItem(var0, var9, var5, var12, false);
										}
									}
								}

								if (class268.method5042(WorldMapIcon_0.getWidgetClickMask(var0))) {
									GraphicsObject.insertMenuItemNoShift("Use", PacketWriter.colorStartTag(16748608) + var9.name, 38, var9.id, var5, var0.id);
								}
								...
								if (WorldMapManager.method673(WorldMapIcon_0.getWidgetClickMask(var0))) {
									...
									if (var11 >= 0) {
										GrandExchangeEvents.addWidgetItemMenuItem(var0, var9, var5, var11, true);
									}
								}
								...
								if (var10 != null) {
									for (var12 = 4; var12 >= 0; --var12) {
										if (var10[var12] != null) {
											...
											GraphicsObject.insertMenuItemNoShift(var10[var12], PacketWriter.colorStartTag(16748608) + var9.name, var13, var9.id, var5, var0.id);
										}
									}
								}

								GraphicsObject.insertMenuItemNoShift("Examine", PacketWriter.colorStartTag(16748608) + var9.name, 1005, var9.id, var5, var0.id);
							}
						}
					}
		...
		if (var0.isIf3) {
			if (Client.isSpellSelected) {
				...
				for (var5 = 9; var5 >= 5; --var5) {
					var14 = class80.method2098(var0, var5);
					if (var14 != null) {
						GraphicsObject.insertMenuItemNoShift(var14, var0.dataText, 1007, var5 + 1, var0.childIndex, var0.id);
					}
				}
				...
				for (var4 = 4; var4 >= 0; --var4) {
					var14 = class80.method2098(var0, var4);
					if (var14 != null) {
						class160.insertMenuItem(var14, var0.dataText, 57, var4 + 1, var0.childIndex, var0.id, var0.field2713);
					}
				}
