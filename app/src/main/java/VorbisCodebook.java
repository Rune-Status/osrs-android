import net.runelite.mapping.Export;
import net.runelite.mapping.Implements;
import net.runelite.mapping.ObfuscatedName;

@ObfuscatedName("cl")
@Implements("VorbisCodebook")
public class VorbisCodebook {
	@ObfuscatedName("c")
	@Export("dimensions")
	int dimensions;
	@ObfuscatedName("t")
	@Export("entries")
	int entries;
	@ObfuscatedName("o")
	@Export("lengthMap")
	int[] lengthMap;
	@ObfuscatedName("e")
	int[] field1334;
	@ObfuscatedName("i")
	float[][] field1335;
	@ObfuscatedName("g")
	@Export("keys")
	int[] keys;

	VorbisCodebook() {
		VorbisSample.readBits(24);
		this.dimensions = VorbisSample.readBits(16);
		this.entries = VorbisSample.readBits(24);
		this.lengthMap = new int[this.entries];
		boolean var1 = VorbisSample.readBit() != 0;
		int var2;
		int var3;
		int var4;
		if (var1) {
			var2 = 0;

			for (var3 = VorbisSample.readBits(5) + 1; var2 < this.entries; ++var3) {
				int var5 = VorbisSample.readBits(CollisionMap.iLog(this.entries - var2));

				for (var4 = 0; var4 < var5; ++var4) {
					this.lengthMap[var2++] = var3;
				}
			}
		} else {
			boolean var15 = VorbisSample.readBit() != 0;

			for (var3 = 0; var3 < this.entries; ++var3) {
				if (var15 && VorbisSample.readBit() == 0) {
					this.lengthMap[var3] = 0;
				} else {
					this.lengthMap[var3] = VorbisSample.readBits(5) + 1;
				}
			}
		}

		this.method2396();
		var2 = VorbisSample.readBits(4);
		if (var2 > 0) {
			float var16 = VorbisSample.float32Unpack(VorbisSample.readBits(32));
			float var6 = VorbisSample.float32Unpack(VorbisSample.readBits(32));
			var4 = VorbisSample.readBits(4) + 1;
			boolean var7 = VorbisSample.readBit() != 0;
			int var8;
			if (var2 == 1) {
				var8 = mapType1QuantValues(this.entries, this.dimensions);
			} else {
				var8 = this.entries * this.dimensions;
			}

			this.field1334 = new int[var8];

			int var9;
			for (var9 = 0; var9 < var8; ++var9) {
				this.field1334[var9] = VorbisSample.readBits(var4);
			}

			this.field1335 = new float[this.entries][this.dimensions];
			float var10;
			int var11;
			int var12;
			if (var2 == 1) {
				for (var9 = 0; var9 < this.entries; ++var9) {
					var10 = 0.0F;
					var11 = 1;

					for (var12 = 0; var12 < this.dimensions; ++var12) {
						int var13 = var9 / var11 % var8;
						float var14 = (float)this.field1334[var13] * var6 + var16 + var10;
						this.field1335[var9][var12] = var14;
						if (var7) {
							var10 = var14;
						}

						var11 *= var8;
					}
				}
			} else {
				for (var9 = 0; var9 < this.entries; ++var9) {
					var10 = 0.0F;
					var11 = var9 * this.dimensions;

					for (var12 = 0; var12 < this.dimensions; ++var12) {
						float var17 = (float)this.field1334[var11] * var6 + var16 + var10;
						this.field1335[var9][var12] = var17;
						if (var7) {
							var10 = var17;
						}

						++var11;
					}
				}
			}
		}

	}

	@ObfuscatedName("t")
	void method2396() {
		int[] var1 = new int[this.entries];
		int[] var2 = new int[33];

		int var3;
		int var4;
		int var5;
		int var6;
		int var7;
		int var8;
		int var9;
		int var10;
		for (var3 = 0; var3 < this.entries; ++var3) {
			var4 = this.lengthMap[var3];
			if (var4 != 0) {
				var5 = 1 << 32 - var4;
				var6 = var2[var4];
				var1[var3] = var6;
				if ((var6 & var5) != 0) {
					var7 = var2[var4 - 1];
				} else {
					var7 = var6 | var5;

					for (var8 = var4 - 1; var8 >= 1; --var8) {
						var10 = var2[var8];
						if (var10 != var6) {
							break;
						}

						var9 = 1 << 32 - var8;
						if ((var10 & var9) != 0) {
							var2[var8] = var2[var8 - 1];
							break;
						}

						var2[var8] = var10 | var9;
					}
				}

				var2[var4] = var7;

				for (var8 = var4 + 1; var8 <= 32; ++var8) {
					var10 = var2[var8];
					if (var10 == var6) {
						var2[var8] = var7;
					}
				}
			}
		}

		this.keys = new int[8];
		var10 = 0;

		for (var3 = 0; var3 < this.entries; ++var3) {
			var4 = this.lengthMap[var3];
			if (var4 != 0) {
				var5 = var1[var3];
				var6 = 0;

				for (var7 = 0; var7 < var4; ++var7) {
					var8 = Integer.MIN_VALUE >>> var7;
					if ((var5 & var8) != 0) {
						if (this.keys[var6] == 0) {
							this.keys[var6] = var10;
						}

						var6 = this.keys[var6];
					} else {
						++var6;
					}

					if (var6 >= this.keys.length) {
						int[] var11 = new int[this.keys.length * 2];

						for (var9 = 0; var9 < this.keys.length; ++var9) {
							var11[var9] = this.keys[var9];
						}

						this.keys = var11;
					}

					var8 >>>= 1;
				}

				this.keys[var6] = ~var3;
				if (var6 >= var10) {
					var10 = var6 + 1;
				}
			}
		}

	}

	@ObfuscatedName("o")
	int method2398() {
		int var1;
		for (var1 = 0; this.keys[var1] >= 0; var1 = VorbisSample.readBit() != 0 ? this.keys[var1] : var1 + 1) {
		}

		return ~this.keys[var1];
	}

	@ObfuscatedName("e")
	float[] method2397() {
		return this.field1335[this.method2398()];
	}

	@ObfuscatedName("c")
	@Export("mapType1QuantValues")
	static int mapType1QuantValues(int var0, int var1) {
		int var2 = (int)Math.pow((double)var0, 1.0D / (double)var1) + 1;

		while (true) {
			int var3 = var2;
			int var4 = var1;

			int var5;
			for (var5 = 1; var4 > 1; var4 >>= 1) {
				if ((var4 & 1) != 0) {
					var5 *= var3;
				}

				var3 *= var3;
			}

			int var6;
			if (var4 == 1) {
				var6 = var3 * var5;
			} else {
				var6 = var5;
			}

			if (var6 <= var0) {
				return var2;
			}

			--var2;
		}
	}
}
